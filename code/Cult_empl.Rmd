...
cult_empl_fig1 <- get_eurostat_data(id1, filters = list(time = date1, sex = "T", unit = "THS_PER", label = T))
...
cult_empl_fig1 <-  cult_empl_fig1 %>%
                            ...
                            filter(!is.na(rate_of_change)) %>% ungroup()
...
# add placeholders for spaces in graphic
...
placeholder_df[, 1:4] <- lapply(placeholder_df[, 1:4], factor)

cult_empl_fig1 <- bind_rows(NA_fig1, placeholder_df, cult_empl_fig1)
...
fig1 <- ggplot(cult_empl_fig1 , aes(x = geo, y = rate_of_change, fill = time)) + 
  ...
  labs(title = paste0("Cultural employment ‒ annual rates of change, ",
                       date1[2],"-",date1[length(date1)]), 
       subtitle = "(%)",
        caption = paste0("Figure 1: Cultural employment ‒ annual rates of change, 2020-2022", "\n",
                         "\n(%)", "\n",
