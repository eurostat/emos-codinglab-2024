...
p7 = ggplot(data = datafig7, aes(x = geo, y = values, fill = ind_type))+
  ...
  labs(
    title = paste0("Young people with at least basic digital skills by gender, ",tail(date7, n = 1)),
    subtitle = "(%)",
...
dsd<-as.data.table(get_eurostat_dsd(id8))[concept=="geo",c("code","name")]

datafig8<-merge(data8, dsd, 
                by.x="geo",by.y="name", 
                all.x=T, all.y = F)
...
name_ord <- datafig8[(code %in% eu_cc) & (unit == "Percentage of individuals") & (ind_type == "Females")]

name_ord_eu<-name_ord[order(values)]$geo

name_ord<-datafig8[(code %in% efta_cc) & (unit == "Percentage of individuals") & (ind_type == "Females")]

name_ord_efta<-name_ord[order(values)]$geo

name_ord<-datafig8[!(code %in% c(efta_cc,eu_cc,"EU27_2020")) & (unit == "Percentage of individuals") & (ind_type == "Females")]
...
ggsave("digital_figures/p8.png", plot = p8, width = 12, height = 8, dpi = 1000)
knitr::include_graphics("digital_figures/p8.png")
